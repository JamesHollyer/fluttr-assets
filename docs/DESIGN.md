...
**2026-09-21**
- **Account deletion** (App Store requirement): a Delete account card on the Account screen with a two-step confirmation. A `delete_account()` server function removes the auth row; every user table cascades from it, so profile, catches, badges, friendships, cheers, comments, notifications, and device tokens go with it. The phone then wipes its local copy of the user's catches and badges so nothing is re-uploaded on a later sign-in. Verified end to end by deleting the `finch_dev` test account.
- The Sound ID model is now served from a public GitHub release (`JamesHollyer/fluttr-assets`) instead of the developer's Mac; builds after 2026-09-21 download it from there. Cloudflare R2 behind a custom domain is the planned replacement once the project has a domain.
